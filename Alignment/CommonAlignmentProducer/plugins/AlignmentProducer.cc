...
AlignmentProducer::AlignmentProducer(const edm::ParameterSet& iConfig) :
  ...
  recoMuonCollectionTag_(iConfig.getParameter<edm::InputTag>("recoMuonCollectionTag")),
  recoTrackCollectionTag_(iConfig.getParameter<edm::InputTag>("recoTrackCollectionTag"))
{
			       ...
			       const edm::EventSetup& setup )
{
  ...
  if (event.getByLabel(tjTkAssociationMapTag_, m_TrajTracksMap)) {
    ...
    event.getByLabel(recoMuonCollectionTag_, recoMuons);
    
    edm::Handle<reco::TrackCollection> recoTracks;
    event.getByLabel(recoTrackCollectionTag_, recoTracks);
    
    // Run the alignment algorithm with its input
    const AlignmentAlgorithmBase::EventInfo eventInfo(event.id(), trajTracks, *beamSpot,
						      clusterValueMapPtr, *recoMuons, *recoTracks);
