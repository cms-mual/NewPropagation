...
AlignmentMonitorMuonSystemMap1D::AlignmentMonitorMuonSystemMap1D(const edm::ParameterSet& cfg)
   ...
   , m_trackCollectionTag(cfg.getParameter<edm::InputTag>("trackCollectionTag"))
...
{
  std::cout << "AlignmentMonitorMuonSystemMap1D::event BEGIN" << std::endl;
  m_counter_event++;
  ...
  if (!m_useMuons) // use trajectories (old approach)
  {
  ...
  else
  {
    ...
    iEvent.getByLabel(m_muonCollectionTag, recoMuons);
    
    edm::Handle<reco::TrackCollection> generalTracks;
    iEvent.getByLabel(m_trackCollectionTag, generalTracks);
    ...
    if ( recoMuons->size() > 0 && generalTracks->size() > 0 ) {
      for (reco::MuonCollection::const_iterator muon = recoMuons->begin();  muon != recoMuons->end();  ++muon) {
        ...
        if (    muon->isGlobalMuon()
	        ...
	        if ( fabs( muon->innerTrack()->dxy( beamSpot->position() ) ) < 0.2 ) {
            m_counter_trackdxy++;
            ...
            for (reco::TrackCollection::const_iterator track = generalTracks->begin();  track != generalTracks->end();  ++track) {
              if (    track->pt()                                >  30.0
                   ...
                   && fabs( track->dxy( beamSpot->position() ) <  0.2 )
	            ) {
	                
                if ( fabs ( muon->innerTrack()->pt() - track->pt() ) < 0.1 && fabs ( muon->innerTrack()->eta() - track->eta() ) < 0.01 ) {
                
                  MuonResidualsFromTrack muonResidualsFromTrack(iSetup, magneticField, globalGeometry, prop, &*muon, &*track, pNavigator(), 1000.);
                  processMuonResidualsFromTrack(muonResidualsFromTrack, iEvent);
...
// End: Vadim's code
  ...
  std::cout << "AlignmentMonitorMuonSystemMap1D::event END" << std::endl;
}
