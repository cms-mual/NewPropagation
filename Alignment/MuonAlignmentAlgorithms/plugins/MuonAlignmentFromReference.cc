...
MuonAlignmentFromReference::MuonAlignmentFromReference(const edm::ParameterSet &cfg)
  ...
  , m_useMuons(cfg.getParameter<bool>("useMuons"))
...
void MuonAlignmentFromReference::run(const edm::EventSetup& iSetup, const EventInfo &eventInfo)
{
  ...
  if (!m_useMuons) // use trajectories
  {
    std::cout << "MuonAlignmentFromReference::run USE TRAJECTORIES START" << std::endl;
    ...
    std::cout << "MuonAlignmentFromReference::run USE TRAJECTORIES STOP" << std::endl;
    
  }
  else // use muons
  {
    std::cout << "MuonAlignmentFromReference::run USE MUONS START" << std::endl;
    reco::MuonCollection recoMuons = eventInfo.muonCollection();
    reco::TrackCollection generalTracks = eventInfo.trackCollection();
    
    if ( recoMuons.size() > 0 && generalTracks.size() > 0 ) {
      for (reco::MuonCollection::const_iterator muon = recoMuons.begin();  muon != recoMuons.end();  ++muon) {
        ...
        if (    muon->isGlobalMuon()
	        ...
	        if ( fabs( muon->innerTrack()->dxy( eventInfo.beamSpot().position() ) ) < 0.2 ) {
            m_counter_trackdxy++;
            
            for (reco::TrackCollection::const_iterator track = generalTracks.begin();  track != generalTracks.end();  ++track) {
              if (    track->pt()                                >  30.0
                   && track->pt()                                <  200.0
                   && track->normalizedChi2()                    <  10.0
                   && track->numberOfValidHits()                 >= 10
                   && fabs( track->dxy( eventInfo.beamSpot().position() ) <  0.2 )
	            ) {
	                
                if ( fabs ( muon->innerTrack()->pt() - track->pt() ) < 0.1 && fabs ( muon->innerTrack()->eta() - track->eta() ) < 0.01 ) {
                  MuonResidualsFromTrack muonResidualsFromTrack(iSetup, magneticField, globalGeometry, prop, &*muon, &*track, m_alignableNavigator, 1000.);
                  processMuonResidualsFromTrack(muonResidualsFromTrack);
                }
	            }
            }
      ...
      std::cout << "MUON or TRACK COLLECTIONS ARE NOT VALID" << std::endl;
    }

    std::cout << "MuonAlignmentFromReference::run USE MUONS STOP" << std::endl;
    
// Old Vadim's code BEGIN
...
// Old Vadim's code END

  }
  
  std::cout << "MuonAlignmentFromReference::run STOP <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << std::endl;
...
{
  std::cout << "MuonAlignmentFromReference::processMuonResidualsFromTrack START" << std::endl;
  
  std::cout << "mrft.trackerNumHits(): " << mrft.trackerNumHits() << " m_minTrackerHits: " << m_minTrackerHits << std::endl;
  if (mrft.trackerNumHits() >= m_minTrackerHits)
  {
    m_counter_trackerhits++;
    
    std::cout << "mrft.normalizedChi2(): " << mrft.normalizedChi2() << " m_maxTrackerRedChi2: " << m_maxTrackerRedChi2 << std::endl;
    if (mrft.normalizedChi2() < m_maxTrackerRedChi2)
    {
      m_counter_trackerchi2++;
      
      std::cout << "mrft.contains_TIDTEC(): " << mrft.contains_TIDTEC() << " m_allowTIDTEC: " << std::endl;
      if (m_allowTIDTEC  ||  !mrft.contains_TIDTEC())
      {
        m_counter_trackertidtec++;

        std::vector<DetId> chamberIds = mrft.chamberIds();
        std::cout << "mrft.chamberIds().size(): " << mrft.chamberIds().size() << " m_minNCrossedChambers: " << m_minNCrossedChambers << std::endl;
        if ((int)chamberIds.size() >= m_minNCrossedChambers)
        {
          ...
          std::cout << "Start loop over chambers" << std::endl;
          for (std::vector<DetId>::const_iterator chamberId = chamberIds.begin();  chamberId != chamberIds.end();  ++chamberId)
          {
            ...
            std::cout << "chamberId->rawId(): " << chamberId->rawId() << " chamberId->subdetId(): " << chamberId->subdetId() << " MuonSubdetId::DT: " << MuonSubdetId::DT << " MuonSubdetId::CSC: " << MuonSubdetId::CSC << std::endl;
            ...
            {
              std::cout << "This is DT station 1-3" << std::endl;
              ...
              {
                std::cout << "dt13 and dt2 are valid" << std::endl;
                m_counter_station123valid++;
                std::cout << "dt13->numHits(): " << dt13->numHits() << " m_minDT13Hits: " << m_minDT13Hits << std::endl;
                if (dt13->numHits() >= m_minDT13Hits)
                {
                  m_counter_station123dt13hits++;
                  std::cout << "dt2->numHits(): " << dt2->numHits() << " m_minDT2Hits: " << m_minDT2Hits << std::endl;
                  if (dt2->numHits() >= m_minDT2Hits)
                  {
                    m_counter_station123dt2hits++;
                    std::cout << "Find this chamber in alignable map" << std::endl;
                    ...
                    if (fitter != m_fitters.end())
                    {
                      ...
                      std::cout << "dt2->resslope(): " << dt2->resslope() << " m_maxResSlopeY: " << m_maxResSlopeY << std::endl;
            ...
            // DT 4th station
                     ...
                     DTChamberId(chamberId->rawId()).station() == 4)
            {
              std::cout << "This is DT station 4" << std::endl;
              MuonChamberResidual *dt13 = mrft.chamberResidual(*chamberId, MuonChamberResidual::kDT13);

              m_counter_station4++;
              if (dt13 != NULL)
              {
                std::cout << "dt13 is valid" << std::endl;
            ...
            {
              std::cout << "This is CSC" << std::endl;
              MuonChamberResidual *csc = mrft.chamberResidual(*chamberId, MuonChamberResidual::kCSC);
              m_counter_csc++;
              if (csc != NULL)
              {
                std::cout << "csc is valid" << std::endl;
                m_counter_cscvalid++;
                std::cout << "csc->numHits(): " << csc->numHits() << " m_minCSCHits: " << m_minCSCHits << std::endl;
  ...
  std::cout << "MuonAlignmentFromReference::processMuonResidualsFromTrack STOP" << std::endl;
}
  ...
  for (std::vector<Alignable*>::const_iterator ali = m_alignables.begin(); ali != m_alignables.end(); ++ali)
  {
    ...
    if (fitter != m_fitters.end())
    {
      ...
      if (fitter->second->numsegments() >= m_minAlignmentHits)
      {
        ...
        // FIXME! Yuriy: Line below is commented out to avoid problems at validation step
