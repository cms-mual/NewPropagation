...
MuonResidualsFromTrack::MuonResidualsFromTrack(const edm::EventSetup& iSetup,
                                               ...
                                               double maxResidual)
  : m_recoTrack(recoTrack), m_recoMuon(recoMuon)
{
  std::cout << "BEGIN MuonResidualsFromTrack: muon input" << std::endl;
...
//  double lastTrackerTsosGlobalPositionR = 0.0;
  ...
  TrajectoryStateOnSurface track_OuterHit_TSOS;
    
  int nHit      = 0;
  int nHitValid = 0;
  
  m_tracker_chi2 = m_recoTrack->normalizedChi2();
  m_tracker_numHits = m_recoTrack->numberOfValidHits();
  
  for ( trackingRecHit_iterator hit = m_recoTrack->recHitsBegin(); hit != m_recoTrack->recHitsEnd();  ++hit ) {
    
    std::cout << " Hit " << nHit << " is found. Validity: " << (*hit)->isValid() << std::endl;
    ...
    if( (*hit)->isValid() ) {
      ...
      if ( hitId.det() == DetId::Tracker ) {
        std::cout << "  The hit is in Tracker" << std::endl;
        std::cout << "    The hit dimension: " << (*hit)->dimension() << std::endl;
        ...
        GlobalPoint hit_GP = globalGeometry->idToDet(hitId)->toGlobal( (*hit)->localPosition() );
        ...
        float hit_GP_R = sqrt( hit_GP.x() * hit_GP.x() + hit_GP.y() * hit_GP.y());
        std::cout << "    The hit global R: " << hit_GP_R << std::endl;
      } else if (hitId.det() == DetId::Muon) {
        std::cout << "  Warning! General track has hit in Muon system" << std::endl;
      } else {
        std::cout << "  Warning! General track has hit in Unknown system" << std::endl;
      }
  ...
  GlobalPoint                          track_OuterHit_Position         = GlobalPoint(m_recoTrack->outerPosition().x(), m_recoTrack->outerPosition().y(), m_recoTrack->outerPosition().z());
  GlobalVector                         track_OuterHit_Momentum         = GlobalVector(m_recoTrack->outerMomentum().x(), m_recoTrack->outerMomentum().y(), m_recoTrack->outerMomentum().z());
  int                                  track_Charge                    = m_recoTrack->charge();
  GlobalTrajectoryParameters           track_OuterHit_GlobalTrajParams = GlobalTrajectoryParameters(track_OuterHit_Position, track_OuterHit_Momentum, track_Charge, &(*magneticField));
  const reco::Track::CovarianceMatrix  track_OuterHit_CovMatr          = m_recoTrack->outerStateCovariance();
  CurvilinearTrajectoryError           track_OuterHit_CurviTrajErr     = CurvilinearTrajectoryError(track_OuterHit_CovMatr);
  DetId                                track_OuterHit_DetId            = DetId(m_recoTrack->outerDetId());
  
//  edm::ESHandle<TrackerGeometry> trackerGeometry;
//  iSetup.get<TrackerDigiGeometryRecord>().get(trackerGeometry);
//  track_OuterHit_TSOS = TrajectoryStateOnSurface(track_OuterHit_GlobalTrajParams, track_OuterHit_CurviTrajErr, trackerGeometry->idToDet(track_OuterHit_DetId)->surface());
  track_OuterHit_TSOS = TrajectoryStateOnSurface(track_OuterHit_GlobalTrajParams, track_OuterHit_CurviTrajErr, globalGeometry->idToDet(track_OuterHit_DetId)->surface());
  
  lastTrackerTsos = track_OuterHit_TSOS;
...
//          std::cout << "        TrajectoryMeasurement hit subDet: DT wheel " << chamberId.wheel() << " station " << chamberId.station() << " sector " << chamberId.sector() << std::endl;
// 
...
//          std::cout << "        TrajectoryMeasurement hit subDet: CSC endcap " << cscDetId.endcap() << " station " << cscDetId.station() << " ring " << cscDetId.ring() << " chamber " << cscDetId.chamber() << std::endl;
//          std::cout << "        TrajectoryMeasurement hit dimension: " << trajMeasurementHitDim << std::endl;
//          
...
//  }    
  ...
  const reco::Track* outerTrack = m_recoMuon->outerTrack().get();
  
  int iT2 = 0, iM2 = 0;
//  for (trackingRecHit_iterator hit2 = m_recoTrack->recHitsBegin(); hit2 != m_recoTrack->recHitsEnd(); ++hit2) {
  for (trackingRecHit_iterator hit2 = outerTrack->recHitsBegin(); hit2 != outerTrack->recHitsEnd(); ++hit2) {
    if((*hit2)->isValid()) {
...
//        recHitsForRefit.push_back(theMuonRecHitBuilder->build(&**hit));
  ...
  std::cout << "END MuonResidualsFromTrack: muon input" << std::endl << std::endl;
}
...
{
  std::cout << "BEGIN MuonResidualsFromTrack: trajectory input" << std::endl;
  const std::string metname = " *** MuonResidualsFromTrack *** ";
  LogTrace(metname) << "Tracking Component changed!";
...
//        recHitsForRefit.push_back(theMuonRecHitBuilder->build(&**hit));
  ...
  std::cout << "END MuonResidualsFromTrack: trajectory input" << std::endl << std::endl;
}
